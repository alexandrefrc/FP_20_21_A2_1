...
for index, year_results in complete_info_df.groupby('Year'):
    ...
    year_results['Average Ratio'] = (year_results['Snatch/Weight']+year_results['Clean&Jerk/Weight'])/2
    ...
    year_results['Position by ratio'] = year_results['Average Ratio'].rank(ascending=False, method='first')
...
print('All time highest average ratio: {}'.format(round(position_by_ratio_df['Average Ratio'].max(), 3)))
print('Average Ratio: {}'.format(round(position_by_ratio_df['Average Ratio'].mean(), 3)))
print('Standard Deviation: {}\n'.format(round(position_by_ratio_df['Average Ratio'].std(), 5)))

# GRAPH 1
# Gráfico médias atletas e médias medalhistas

avg_all_athletes = complete_info_df.groupby('Year').Score.mean().plot(xticks=complete_info_df['Year'].unique(), style='o-')
...
for pair in [(xs_athletes, ys_athletes),(xs_medalists, ys_medalists)]:
    for x, y in zip(pair[0], pair[1]):
        label = "{:.2f}".format(y)
        plt.annotate(label, (x, y), textcoords="offset points", xytext=(0, 10), bbox=dict(boxstyle="round", fc="0.9", ec="gray"), ha='center')
print('\nAverage Score per Year graph:')
plt.show()
print('Average score in all editions: {}'.format(round(complete_info_df['Score'].mean(), 3)))
print('Average medalists score in all editions: {}\n'.format(round(medalists['Score'].mean(), 3)))
...
plt.show()
print('Average difference between first and last place: {}'.format(sum(first_last)/len(first_last)))
...
print('Average difference between first and third place: {}'.format(sum(first_third)/len(first_third)))
...
print('\nAverage failed Snatch lifts: {}'.format(round(failed_lifts_df['Average All Athletes','Failed Snatch'].mean(), 3)))
print('Average failed Clean and Jerk lifts: {}'.format(round(failed_lifts_df['Average All Athletes','Failed Clean and Jerk'].mean(), 3)))
print('Average failed lifts: {}'.format(round(failed_lifts_df['Average All Athletes','Overall Failed'].mean(), 3)))
print('\n{} has the lower average of failed lifts: {}'.format(failed_lifts_df[failed_lifts_df['Average All Athletes','Overall Failed'] == failed_lifts_df['Average All Athletes','Overall Failed'].min()].index[0].astype(int), round(failed_lifts_df['Average All Athletes','Overall Failed'].min(),3)))
print('{} has the highest average of failed lifts: {}\n'.format(failed_lifts_df[failed_lifts_df['Average All Athletes','Overall Failed'] == failed_lifts_df['Average All Athletes','Overall Failed'].max()].index[0].astype(int), round(failed_lifts_df['Average All Athletes','Overall Failed'].max(),3)))
...
ages = ages.reindex(pd.RangeIndex(ages.index.min(), ages.index.max() + 1)).fillna(0)
...
print('Most common age of participation is {} with {} participants.'.format(ages[ages==ages.max()].index[0].astype(int), int(ages.max())))
plt.show()
